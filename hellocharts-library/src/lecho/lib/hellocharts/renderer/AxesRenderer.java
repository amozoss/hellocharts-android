...
public class AxesRenderer {
    ...
    private void initAxisPaints(Axis axis, int position) {
        ...
        goalPaint.setColor(axis.getGoalLineColor());
    ...
    private void drawGoalLine(Canvas canvas, Axis axis, int position) {
        if (!axis.hasGoalLine()) { return; }

        final Rect contentRectMargins = computator.getContentRectMinusAxesMargins();
        goalPaint.setStrokeWidth(ChartUtils.dp2px(density, axis.getGoalLineStrokeWidth()));
        if (position == LEFT || position == RIGHT) {
            float goalLineYPosition = computator.computeRawY(axis.getGoalLineValue());
            canvas.drawLine(contentRectMargins.left, goalLineYPosition, contentRectMargins.right, goalLineYPosition, goalPaint);
        }  else if (position == BOTTOM || position == TOP) {
            float goalLineXPosition = computator.computeRawX(axis.getGoalLineValue());
            canvas.drawLine(goalLineXPosition, contentRectMargins.top, goalLineXPosition, contentRectMargins.bottom, goalPaint);
        }
    }
    ...
    private void drawAxisLabelsAndName(Canvas canvas, Axis axis, int position) {
        ...
        for (int valueToDrawIndex = 0; valueToDrawIndex < valuesToDrawNumTab[position]; ++valueToDrawIndex) {
            ...
            if (axis.isAutoGenerated()) {
                ...
                if (value == axis.getGoalLineValue()) {
                    drawGoalColor = true;
                }
            ...
            if (axis.hasTiltedLabels()) {
                ...
                int beforeColor = labelPaintTab[position].getColor();
                if (drawGoalColor) {
                    labelPaintTab[position].setColor(axis.getGoalLineColor());
                }
                canvas.drawText(labelBuffer, labelBuffer.length - charsNumber, charsNumber, labelX, labelY,
                        labelPaintTab[position]);
                if (drawGoalColor) {
                    labelPaintTab[position].setColor(beforeColor);
                }
            }
        }
